...
import { statusStudentApi } from "../../../../../../services/ReactLearnTwoService"
...
export function WithoutAsyncAndService({ getData }: any) {
  ...
  const statusStudent = (event: any) => {
    event.preventDefault()
    axios.patch(`https://kisalayakgschool.com/api/statusStudent/${targetId}`, {},
      {
        headers: {
...
export function WithoutAsyncButWithService({ getData }: any) {
  ...
  const statusStudent = (event: any) => {
    event.preventDefault()
    statusStudentApi(targetId).then((res) => {
      console.log(res)
      getData()
    }).catch((err) => console.log(err))
  }
...
export function WithAsyncButWithoutService({ getData }: any) {
  const [targetId, setTargetId] = useState(0)
  const statusStudent = async (event: any) => {
    event.preventDefault()
    const resp = await axios.patch(`https://kisalayakgschool.com/api/statusStudent/${targetId}`, {},
      {
        headers: {
          'X-Mashape-Key': 'required',
          'Content-Type': 'application/x-www-form-urlencoded',
...
export function WithAsyncAndService({ getData }: any) {
  ...
  const statusStudent = async (event: any) => {
    event.preventDefault()
    const resp = await statusStudentApi(targetId)
    if (resp.data.status == 1) {
      getData()
    }
  }
